...
anywhere. That is one of the reasons the allowlist approach was abandoned.
...
Watch the repair, too. `review`'s replacement reads "a change has to exist
first". It excludes a *narrower* case than the sentence it replaced, which
excluded report-only work. That is a deliberate widening, not a restatement.
...
bug that survived a first fix"*, which is `tracer`'s trigger near-verbatim.
...
The v0.9.0 contradiction sweep could not see this. It compared rules, and these
rules did not conflict. The *triggers* overlapped. Two components can be
...
1.6× the largest observed run. That is not an order of magnitude, and it should
not be described as one. To settle it properly, build a throwaway agent with
...
output-style frontmatter. It reports a condition rather than a verdict.
...
- `${CLAUDE_PLUGIN_ROOT}` in a `SKILL.md` body is the established convention,
  with 37 uses across installed third-party plugin skills.
...
fired for its own work. A live test caught that, producing a correct fix with
...
- **YAGNI vs "full solution, every edge case".** Different axes. The ladder
  governs *surface*, which is what to build. Completeness governs *depth*: once
  agreed, it ships whole. "Cutting a feature is a decision to state; cutting error
...
such as a local install or something bundled with Claude Code), or `internal`
(our own review found it). An origin missing from that table fails the check by design, so
...
`UPSTREAM.tsv`. Matching across them loosely produced ten false positives.
...
rewriting. It also contradicted the plugin's own "causes, not symptoms", since
...
the real reason `verification-planning` and `codemap` did not fire, and **not**
the wording. Two description rewrites were spent on the wrong cause before checking
...
Every agent denies `Agent` and `Task`. That was inherited from both upstreams and
adopted without independent testing: omo-slim's fixer says "NO spawning
subagents", and fable-mode says "keep delegation one level deep, nesting
multiplies cost and scatters context". Tested properly on 2026-08-13:
...
cause was already known and already fixed, but only for skills. v0.5.x added a
skill roster to the output style because skill descriptions get dropped on machines
...
that phase only." It contradicts the adopted `Todo continuity` rule, under which
a new task is appended and never substituted. That rule exists to stop earlier
work vanishing from the list. Ours is the better rule; the pin moved without
taking this one.
...
(`RESEARCH.md:100`), so it is off by default there too.
...
cap mid-work, with no stage map, which is the exact failure the skill prevents.
The gap is recognition to action.
...
Documented as manual rather than patched further. The README previously listed
it as auto-firing on the strength of one prompt: "migrate five services, **in
phases**". That prompt contains the trigger word, so it was measuring the prompt,
not the skill.

### The four fixes, kept on their own merits

1. **Cross-cutting trigger** in the roster and the skill description, for a fix
   that must land across several subsystems at once because correcting one layer alone
...
servers, every one of them user-scope**. So inheritance itself works and the
README claim was accurate. But everything it inherits lives at the scope the
prompts told it to skip.
...
- **Principles by name**: KISS, YAGNI, DRY, single responsibility, linear flow,
  modularity. Each is stated with the specific failure it prevents rather than as
  a slogan.
...
the same reason is duplicated knowledge, and code that merely looks alike is
coincidence. The **rule of three** is the explicit tiebreaker, because a wrong
abstraction costs more than the duplication it replaced.
...
`simplify` match unmistakable task *shapes* like "map this repo" and "this is
over-engineered". Meanwhile deepwork asks for a judgement about whether work is
hard enough to stage, and that judgement resolves toward "I can handle this".
...
The replacement is an unconditional imperative plus a named cost of skipping:
"**Read `checklists.md` now, before judging anything.** ... A review that never
opened it is a review running on recall". That moved the read rate from **0/1 to
...
The restructure below is therefore justified on dependency grounds, not on a
measured quality gain: do not put a lane's only checklist behind a coin flip.
...
fresh subagent ... that step is weaker than the skill intends". That is the
...
the public constructor has `total === undefined`, and `500 > undefined` is
`false`, so the only protection in the function approves everything. That is the
...
`findOrders`". That is the confidence axis used as designed rather than as
decoration.
...
so 1 of 2. A lane whose entire checklist is behind a coin flip is not a lane, so the
...
- The interface, data/schema, API contract and operations lanes have never
  fired. The fixture has no migration, route or UI.
...
It is also more automated, not just shorter. Scope is now one bash block that
prints the merge base, the changed-line count and the file list, and lane selection is a
...
none left, further reduction is a content decision — dropping lanes or rules —
and that belongs to whoever owns the capability, not to a compression pass.
...
6/6 planted defects, 2/2 false-positive baits avoided, 0 false positives. The
run also did something no earlier one had: after fixing `statusLabel`, it **added the
...
principles: **comment volume is its own smell**, and **a comment is a fence
too**. Delete a restating comment freely, but `git blame` one you do not understand,
...
deleted subtitles behind a `subtitle` prop, because removing them was the
request. That is scope fidelity holding in the direction that usually fails
quietly.
...
inventory. It is absent from the diff by construction, which makes it
...
It opens with the reason it exists: a member the change forgot is not in the diff,
...
- on a diff over the fan-out threshold, lanes are dispatched one subagent each
  from the lane table, and a subsection is never dispatched;
...
guard added by `fix: NaN from the vendor feed was corrupting invoice totals`,
with the prompt explicitly inviting a tidy-up of that file.
...
and reported honestly that nothing was compiled because the repo has no
toolchain. It then proposed the test that would pin the retry count and spacing.
...
It also exceeded the one-check-behind floor. It wrote 14 tests, ran them, and
...
the exact tool call named, an ordering constraint, escapes closed by name. I
...
4. **The contradiction sweep**, over the shipped prompt surface. It is a release
   ...
   introduced by that release, and every one of them passed every presence check.

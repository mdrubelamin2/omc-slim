...
This entry was written into v0.13.0 by mistake and moved here. v0.13.0 was
already tagged and on the remote; the check that missed it was `git tag | tail`,
where `v0.13.0` sorts before `v0.9.0` and every v0.1x tag fell off the end. A
skill removed from a published roster is a release, not an amendment to one.

...
If the user answers vaguely, note the dimension is still open and ask once more with a sharper question. **Two asks per dimension, and that is the cap**: the original and the sharper one. A third is the interview arguing with the user: record the dimension as an accepted unknown and move on. Outcome is the exception: keep asking it, differently, until the round cap.
...
```
Outcome     3 → 1   "a CLI for the ops team"
Scope       2 → 2   (still open: nothing names what is excluded)
Users       1 → 0
...
...
**An answer that declines to choose is neither approval nor refusal, and it has an exit.** "You decide", "up to you", "whatever you think", silence on the question: the same move, however it is worded. Restate the single riskiest line in the spec, the one that costs most to get wrong. And ask once for an explicit yes to that line alone. **Declined again, stop and hand back the spec path with a three-line summary.** A shrug is not consent: the commonest way a user says "I have not read it yet" is "up to you", and building on that is the failure this gate exists to prevent. Record the unanswered question in the spec's Assumptions so the next session inherits it. A gate with no exit is a stall and costs the user their day — but the exit is ending the turn, not starting the work.

**That is measured, and the measurement is small.** A four-skill pack scored **+1.50 over bare agent**, which is noise; adding **this gate** took it from 83.17 to **97.67**. It is the best-supported number anyone has published for a prompt layer and it is n=3 on one frozen task, on another harness, with a post-hoc rerun — under this repository's own power standard (RESEARCH §8.1: >30pp minimum detectable effect at 17 tasks) a one-task design cannot carry a 14.5pp claim. Keep the gate for the mechanism, not for the number. (Source: docs/RESEARCH-2026-08-26.md §11.9.)

**The gate survives the argument that the user already approved.** They approved *the interview*. They have not seen the spec, because it did not exist when they typed. Section 1's "an explicit invocation is a decision already made" licenses you to skip the *scoring debate*, never the stop. The declined-choice rule above is the one licensed exit. If Section 1 and this gate ever seem to conflict, this one wins.

...
**That is now measured, and it is the largest effect anyone has published for a prompt layer.** In a control-armed, twice-blind-judged comparison, a four-skill pack scored **+1.50 over bare agent**, which is noise. Five methods, one frozen underspecified task, same model and rubric. Adding **this gate** took it from 83.17 to **97.67: +14.50 points for +0.60M tokens and about two minutes**. The full fourteen-skill framework did not beat that pack plus its loops, while spending 7.6× the control arm's tokens. (Source: docs/RESEARCH-2026-08-26.md §11.9.)

...
**Where the request covers a set, review the set, not the diff.** Every page, all the endpoints, each consumer. A missed member is absent from the diff by definition, so the completeness lane is the only one that can reach it. Account for every member the change did not touch.
...
- [REQUIRED] (7/10) file:line  problem
  Fix: the specific change
...
Clean is `Review: ship. 0 findings.` plus the `Lanes:` line, and DROPPED if any candidates were filtered. Then stop. A review that dropped everything it considered is not one that found nothing, and that difference is the reader's to judge.

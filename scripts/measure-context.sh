...
printf 'wrong about what it measures. That command excludes the output style, which\n'
printf 'is the largest single item here, so it reads much smaller.\n\n'
printf 'But it also counts something THIS script does not, and that direction\n'
printf 'matters more. Measured 2026-08-29 against the installed v0.9.1: our basis\n'
printf 'gives 962 tokens for the twelve descriptions and the harness reports 1,461\n'
printf 'always-on for the same twelve — about 42 tokens each of framing (the name,\n'
printf 'the type, the list structure) that a text measurement cannot see.\n\n'
printf 'So the figure above is a FLOOR, not the whole charge. It is not folded in,\n'
printf 'because 42 rests on one observation of an estimate the harness itself calls\n'
printf 'an estimate — and this repository has already published one wrong number by\n'
printf 'baking a single-observation constant into a headline. Re-derive it with:\n'
printf '  claude plugin details omc-slim\n'

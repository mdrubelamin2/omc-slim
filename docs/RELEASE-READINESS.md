...
The version stands at **v0.14.0**. Calling it v1.0 would require either running
...
| 3 | Inertness is visible | **NOT MET — the mechanism was removed in v0.12.0** | The `check-output-style` hook and `scripts/check-adversarial.sh` passed 9/9 against a real rival plugin, and both were deleted in v0.12.0 along with the other SessionStart work. Nothing now reports a stolen style slot to the user. The README states this and gives the manual probe (`claude -p "which output style is active?"`), and `scripts/optional/statusline.sh` still prints the badge for anyone who wires it up. Scored NOT MET rather than descoped: a criterion whose evidence was deleted is not a criterion that passed. The other half, a session whose `Agent` tool is gated, was always unrun. |
...
| 6 | The surface ratchet holds | **MET, against a declared ceiling** | The old rule measured against 4,197, the lowest figure any release had reached. At 2,703 the plugin sat 1,494 under it, so the ratchet had stopped constraining anything — a floor you are nowhere near is not a budget. It is now a **3,000-token ceiling on the always-on surface**, enforced by `check-coverage.sh`, which fails the build rather than reporting: a ceiling nobody enforces is a preference. The static surface is 2,703, 297 under the 3,000 ceiling. Every future addition spends against that 297, and raising the ceiling is a decision someone has to write down. See [CRITERIA-AUDIT-2026-08-29.md](./CRITERIA-AUDIT-2026-08-29.md) §1. |

...
| 3 | Inertness is visible | **MET in mechanism, unverified in practice** | The style reports an absent Agent tool; the SessionStart hook names a rival plugin and now its install path; the deliverable hook carries three states. The two adversarial sessions that would prove it have not run. |

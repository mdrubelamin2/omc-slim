...
a terse register and a hard stop before building. The register addresses the most
saturated complaint in the corpus and works where `CLAUDE.md` edits do not,
because an output style is applied by the harness rather than requested of the
model. The stop scores +14.50 points in a control-armed comparison and is the
